...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/core/ngx_cycle.c#L238
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/core/ngx_conf_file.c#L447
...
for (i = 0; cf->cycle->modules[i]; i++) {
    ...
    for ( /* void */ ; cmd->name.len; cmd++) {
        ...
        // 则会走到
        // https://github.com/iamwwc/nginx-debugable/blob/1db517fb71aed6d6fffc8347086f89eb29b83dea/src/http/ngx_http.c#L122
...
conf 字段决定set函数的第三个参数传递什么，如果是 NGX_STREAM_SRV_CONF_OFFSET，则nginx会将此module的srv_conf作为 void *conf 传递
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/1db517fb71aed6d6fffc8347086f89eb29b83dea/src/http/ngx_http.c#L154
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/http/ngx_http_core_module.c#L875
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/http/ngx_http.c#L486
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/stream/ngx_stream.c#L352
...
offset 只有和set配合才有意义，由于nginx有很多需要将配置项存放到结构体的需求，所以nginx提供了形如 ngx_conf_set_foo_slot 的函数，用于批量解决此需求。
...
<https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/os/unix/ngx_process_cycle.c#L139>
...
```c
// https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/os/unix/ngx_process.c#L319
...
case NGX_PROCESS_HELPER:
    ...
    case ngx_signal_value(NGX_SHUTDOWN_SIGNAL):
        ...
        // https://github.com/iamwwc/nginx-debugable/blob/f02e2a734ef472f0dcf83ab2e8ce96d1acead8a5/src/os/unix/ngx_process_cycle.c#L728
        ngx_quit = 1;
        action = ", shutting down";
        break;
...
Linux Kernel 支持 [SO_REUSEPORT](https://github.com/iamwwc/nginx-debugable/blob/8a30c4b12226c92cd317db614172cc6de2795768/src/core/ngx_connection.c#L297)，允许多process accept 同一个socket，socket将按照一定的算法传给不同process
